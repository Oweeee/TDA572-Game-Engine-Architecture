...
bool AvancezLib::init(int width, int height)
{
	...
	TTF_Init();
	font = TTF_OpenFont("data/centipede.ttf", 28); //this opens a font style and sets a size
	if (font == NULL)
	{
...
{
	SDL_Color font_color = { 255, 0, 0 };  // this is the color in rgb format, maxing out all would give you the color white, and it will be your text's color

	SDL_Surface* surf = TTF_RenderText_Solid(font, msg, font_color); // as TTF_RenderText_Solid could only be used on SDL_Surface then you have to create the surface first
